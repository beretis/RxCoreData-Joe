...
class MultipleContextsTestVC: UIViewController {
    ...
    func performParsingOnBgThenAddToChildContext() {
        DispatchQueue.global(qos: .userInitiated).async {
            Thread.sleep(forTimeInterval: 5)
            var parent = self.createRandomParent()
    ...
    func createRandomParent() -> MainEntity {
        let id = String(arc4random_uniform(6000000))
        var parent = MainEntity(id: id, value: id)
        return parent
    }
