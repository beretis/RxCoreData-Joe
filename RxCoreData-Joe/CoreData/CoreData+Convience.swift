//
//  NSManagedObject+Convience.swift
//  pexeso
//
//  Created by Jozef Matus on 04/12/16.
...
public enum RxCoreDataError: Error {
...
extension NSManagedObjectContext {
    ...
    func fetchPersistable<P: Persistable>(_ type: P.Type = P.self, identifier: String) throws -> P {
        ...
        guard entity != nil else { throw RxCoreDataError.coreDataError(message: "object of type \(type) with id of \(identifier)") }
    ...
    public func removeObjects<P: Persistable>(persistables: [P]) {
        persistables.map { $0.toEntity() }.forEach { (entity) in
            self.delete(entity)
        }
    }
}
