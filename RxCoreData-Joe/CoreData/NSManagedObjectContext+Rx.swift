...
public extension Reactive where Base: NSManagedObjectContext {
    ...
    func update<P: Persistable>(_ persistable: P) throws {
        let item = try get(persistable) ?? self.create(P.self)
        persistable.update(item)
        try item.managedObjectContext?.save()
    }

    func update<P: Persistable>(_ persistable: [P]) throws {
        let p = try persistable.map { try get($0) ?? self.create(P.self) }
        p.enumerated().forEach { persistable[$0.offset].update($0.element) }
        try p.first?.managedObjectContext?.save()
    }
}
